...
val_dataloader = torch.utils.data.DataLoader(dataserver_val,
...
def val(model, val_dataloader, epoch, yolo_interval):
    ...
    filename_for_out1 = "performance_numbers_for validation_on" + str(epoch) + "label.txt"
    filename_for_out2 = "performance_numbers_for_validation_on" + str(epoch) + "regres.txt"
        ...
        for iter, data in enumerate(val_dataloader):
            ...
            for ibx in range(im_tensor.shape[0]):
                ...
                for idx in range(num_objects_in_image[ibx]):
                    ...
                    label = np.asarray(bbox_label_tensor[ibx][idx])
            ...
            IOU_YOLO_Tensor = yolo_tensor_flattened.view(im_tensor.shape[0], 36, 5, 8)
            bhp = torch.zeros(im_tensor.shape[0], 36, 5)
            bwp = torch.zeros(im_tensor.shape[0], 36, 5)
            bhg = torch.zeros(im_tensor.shape[0], 36, 5)
            bwg = torch.zeros(im_tensor.shape[0], 36, 5)
            delx = torch.zeros(im_tensor.shape[0], 36, 5)
            dely = torch.zeros(im_tensor.shape[0], 36, 5)
            delxg = torch.zeros(im_tensor.shape[0], 36, 5)
            delyg = torch.zeros(im_tensor.shape[0], 36, 5)
            count = 0
            IOU_value = 0
            confidence_factor = np.zeros(36)
            for batch in range(im_tensor.shape[0]):
                for iou in range(36):
                    for anchor in range(5):
                        if IOU_YOLO_Tensor[batch, iou, anchor, 0] == 1:

                            print("the object score", IOU_YOLO_Tensor[batch, iou, anchor, 0])
                            prob_pred = torch.max((outresh[batch, iou, anchor])[-3:])
                            indprob_pred = torch.argmax((outresh[batch, iou, anchor])[-3:])
                            indprob_ground = torch.argmax((IOU_YOLO_Tensor[batch, iou, anchor])[-3:])
                            if indprob_pred == indprob_ground:
                                count += 1
                                print("the probablity of the class and index ground truth and prediction.........",
                                      prob_pred, indprob_ground, indprob_pred)
                                bwptemp = bwp[batch, iou, anchor] = outresh[batch, iou, anchor, 3]
                                bhptemp = bhp[batch, iou, anchor] = outresh[batch, iou, anchor, 4]
                                delx[batch, iou, anchor] = outresh[batch, iou, anchor, 1]
                                dely[batch, iou, anchor] = outresh[batch, iou, anchor, 2]
                                delxtemp = outresh[batch, iou, anchor, 1]
                                delytemp = outresh[batch, iou, anchor, 2]
                                obj_bb_height = bhptemp * (yolo_interval)
                                obj_bb_width = bwptemp * (yolo_interval)
                                cell_row_idx = iou // 6  ## for the i coordinate
                                cell_col_idx = iou - (
                                            6 * cell_row_idx)  ## for the j coordinates     cell_row_idx = height_center_bb // yolo_interval  ## for the i coordinate
                                yolocell_center_i = cell_row_idx * yolo_interval + float(yolo_interval) / 2.0
                                yolocell_center_j = cell_col_idx * yolo_interval + float(yolo_interval) / 2.0
                                obj_center_x = float(delxtemp * yolo_interval) + float(yolocell_center_j)
                                obj_center_y = float(delytemp * yolo_interval) + float(yolocell_center_i)
                                bboxpred = ([(obj_center_x - (obj_bb_height / 2)), (obj_center_y - (obj_bb_width / 2)),
                                             obj_bb_height, obj_bb_width])
                                print("check check prediction", (bboxpred))
                                print("the bbox prediciton")

                                bwgtemp = bwg[batch, iou, anchor] = IOU_YOLO_Tensor[batch, iou, anchor, 3]
                                bhgtemp = bhg[batch, iou, anchor] = IOU_YOLO_Tensor[batch, iou, anchor, 4]
                                # delxg[batch,iou,anchor]=IOU_YOLO_Tensor[batch,iou,anchor,1]
                                # delyg[batch,iou,anchor]=IOU_YOLO_Tensor[batch,iou,anchor,2]
                                delxtempg = IOU_YOLO_Tensor[batch, iou, anchor, 1]
                                delytempg = IOU_YOLO_Tensor[batch, iou, anchor, 2]
                                obj_bb_heightg = bhgtemp * (yolo_interval)
                                obj_bb_widthg = bwgtemp * (yolo_interval)
                                cell_row_idx = iou // 6  ## for the i coordinate
                                cell_col_idx = iou - (
                                            6 * cell_row_idx)  ## for the j coordinates     cell_row_idx = height_center_bb // yolo_interval  ## for the i coordinate
                                yolocell_center_i = cell_row_idx * yolo_interval + float(yolo_interval) / 2.0
                                yolocell_center_j = cell_col_idx * yolo_interval + float(yolo_interval) / 2.0
                                obj_center_xg = float(delxtempg * yolo_interval) + float(yolocell_center_j)
                                obj_center_yg = float(delytempg * yolo_interval) + float(yolocell_center_i)
                                bboxg = ([(obj_center_xg - (obj_bb_heightg / 2)), (obj_center_yg - (obj_bb_widthg / 2)),
                                          obj_bb_heightg, obj_bb_widthg])
                                print("check check ground truth", (bboxg))
                                print("the bbox round truth", type(bboxpred), type(bboxg))
                                x1 = torch.max(bboxg[0], bboxpred[0])
                                y1 = torch.max(bboxg[1], bboxpred[1])
                                x2 = torch.min((bboxg[0] + bboxg[2]), (bboxg[0] + bboxg[2]))
                                y2 = torch.min((bboxg[1] + bboxg[3]), (bboxpred[1] + bboxpred[3]))
                                intersection = (x2 - x1).clamp(0) * (y2 - y1).clamp(0)
                                boxg_area = bboxg[2] * bboxg[3]
                                boxp_area = bboxpred[2] * bboxpred[3]
                                IOU_value += intersection / ((boxg_area + boxp_area) - intersection)
                                confidence_factor[iou] = prob_pred * intersection / (
                                            (boxg_area + boxp_area) - intersection)
            avgIOU = IOU_value / (count+1e-6)
            print("the avg IOU and confidence value is", avgIOU,confidence_factor)
            if iter % 50 == 49:
    ...
    plt.show()
    # torch.save(model.state_dict(), os.path.join(args.root_path + "valhw6.pth"))



model = model.NetForYolo()
model.load_state_dict(torch.load(os.path.join(args.root_path,"trainmodel.pth")))
val(model, val_dataloader, 1, 20)
